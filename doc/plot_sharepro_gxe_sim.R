...
wdir = "~/Desktop/GitHub/SharePro_gxe_analysis/sim/"
odir = "~/Desktop/GitHub/SharePro_gxe_analysis/doc/"
setwd(wdir)
...
for (i in 1:nrow(SH)) {
  e = cslist[i]
  gem$shp[gem$SNPID %in% unlist(e)]=SH$pdiff[i]
  gem$shtop[gem$SNPID %in% unlist(e)[1]]=SH$pdiff[i]
}
...
ggsave(paste0(odir, "E_UE_association_example.pdf"),plt, height = 5, width = 4)
...
ggplot(pvalplt[pvalplt$Adjustment=="GEM interaction test" | pvalplt$Adjustment=="GEM interaction test (BH-adjusted)",], aes(x = pos / 1000000, y = -log10(P), color=LD)) +
  ...
  scale_color_stepsn(n.breaks = 6, colours = c("darkblue","blue","green","orange","red")) +
  labs(color = expression(r^2)) -> plt
ggsave(paste0(odir, "GEM_GxE_example.pdf"),plt,height = 5, width = 4)
...
ggplot(pvalplt[pvalplt$Adjustment=="SharePro interaction test",], aes(x = pos / 1000000, y = -log10(P), color=LD)) +
  ...
  scale_color_stepsn(n.breaks = 6, colours = c("darkblue","blue","green","orange","red")) +
  labs(color = expression(r^2)) -> plt
ggsave(paste0(odir, "SharePro_GxE_example.pdf"),plt,height = 5, width = 4)
...
ggplot(pipplt[pipplt$Adjustment=="SharePro fine-mapping",], aes(x = pos / 1000000, y = P, color=LD)) +
  ...
  scale_color_stepsn(n.breaks = 6, colours = c("darkblue","blue","green","orange","red")) +
  labs(color = expression(r^2)) -> plt
ggsave(paste0(odir, "SharePro_finemap_example.pdf"),plt,height = 5, width = 4)
...
get_effect_power_fdr <- function(cs,tsnp){
  if (length(cs)==0) {
    power = 0
    fdr = 0
  } else {
    if (length(tsnp)==0) {
      ...
      for (i in 1:length(cs)) {
        e = cslist[i]
        if (length(intersect(unlist(e),tsnp))==1) {
          tp = tp + 1
        }
      }
      power = tp / length(tsnp)
...
Locilist <- c("Locus1/","Locus2/","Locus3/","Locus4/","Locus5/")
  ...
  for (rt in c('CL','CM','CS')) {
    for (lo in Locilist) {
      ...
      for (x in 1:50) {
        ...
        gem$shp <- NA
        totalp = 0
        SHdiff = SH[SH$pdiff<ctf,]
        if (nrow(SH)>0) {
          cslist = strsplit(SH$cs,'/')
          for (i in 1:nrow(SH)) {
            e = cslist[i]
            gem$shp[gem$SNPID %in% unlist(e)]=SH$pdiff[i]
            if (length(intersect(unlist(e),gxesnp))==1 & SH$pdiff[i]<ctf) {
              totalp = totalp + 1
            }
          }
        }
...
#        lpip <- list(SPAp$vProb,
#                     SHp$vProb)
#        pip_pr_vec <- c(pip_pr_vec, unlist(lapply(lpip, function(y){get_cs_power_fdr(gem$SNPID[y>pctf],csnp[x,])})))
#        lcs <- list(SPA$cs,
#                    SH$cs)
#        cs_pr_vec <- c(cs_pr_vec,unlist(lapply(lcs,function(y){get_effect_power_fdr(y,csnp[x,])})))
      }
      pips <- list(spav,
                   shv,
                   gem_pmv,
...
df_auprc$K  = paste0("K:",unlist(lapply(strsplit(df_auprc$folder,'_'),function(x){x[1]})))
df_auprc$Ne = paste0("N[e]:",25000)
df_auprc$Nu = paste0("N[u]:",factor(unlist(lapply(df_auprc$rt, function(x){strsplit(x,2,2)})),
                                           labels = c('25000','10000','5000')))
df_auprc$Be = paste0("beta[e]:",unlist(lapply(strsplit(df_auprc$folder,'_'),function(x){x[2]})))
df_auprc$Bu = paste0("beta[u]:",unlist(lapply(strsplit(df_auprc$folder,'_'),function(x){gsub('/','',x[3])})))

write.table(df_auprc, paste0(odir, "SharePro_gxe_sim_auprc.csv"),sep=',',row.names=F,col.names=T,quote=F)
df_auprc <- read.table(paste0(odir, "SharePro_gxe_sim_auprc.csv"),sep=',',header = T)
df_auprc$method <- factor(df_auprc$method,levels = c("1-df association",
                                                     "2-df association",
                                                     "Combined fine-mapping",
                                                     "SharePro fine-mapping"))
...
df_auprc_median = aggregate(auprc~method+folder+rt,data = df_auprc, median)
df_auprc_median$K = paste0("K:",unlist(lapply(strsplit(df_auprc_median$folder,'_'),function(x){x[1]})))
df_auprc_median$Ne = paste0("N[e]:",25000)
df_auprc_median$Nu = paste0("N[u]:",factor(unlist(lapply(df_auprc_median$rt, function(x){strsplit(x,2,2)})),
                                    labels = c('25000','10000','5000')))
df_auprc_median$Be = paste0("beta[e]:",unlist(lapply(strsplit(df_auprc_median$folder,'_'),function(x){x[2]})))
df_auprc_median$Bu = paste0("beta[u]:",unlist(lapply(strsplit(df_auprc_median$folder,'_'),function(x){gsub('/','',x[3])})))
write.table(df_auprc_median[,c('K','Ne','Nu','Be','Bu','method','auprc')], paste0(odir, "SharePro_gxe_sim_auprc_median.csv"), sep=',',row.names=F,col.names=T,quote=F)

for (k in 1:3) {
  ggplot(df_auprc[startsWith(df_auprc$folder,as.character(k))
                  & df_auprc$method %in% c("Combined fine-mapping","SharePro fine-mapping","1-df association","2-df association"),],
         aes(x=method,y=auprc,color=method)) + 
    geom_point() + 
    ...
    labs(color = "")  -> plt
  ggsave(paste0(odir, "SharePro_gxe_sim_auprc",k,".pdf"),plt,height = 15, width = 10)
}
########
...
write.table(df_gxep_pr,paste0(odir, "SharePro_gxe_sim_gxep_pr.csv"),sep=',',row.names=F,col.names=T,quote=F)
df_gxe_summary = aggregate(cbind(power, fdr) ~ method+rt+folder, data = df_gxep_pr, mean)
df_gxe_summary$K  = paste0("K:",unlist(lapply(strsplit(df_gxe_summary$folder,'_'),function(x){x[1]})))
df_gxe_summary$Ne = paste0("N[e]:",25000)
df_gxe_summary$Nu = paste0("N[u]:",factor(unlist(lapply(df_gxe_summary$rt, function(x){strsplit(x,2,2)})),
                                         labels = c('25000','10000','5000')))
df_gxe_summary$Be = paste0("beta[e]:",unlist(lapply(strsplit(df_gxe_summary$folder,'_'),function(x){x[2]})))
df_gxe_summary$Bu = paste0("beta[u]:",unlist(lapply(strsplit(df_gxe_summary$folder,'_'),function(x){gsub('/','',x[3])})))
df_gxe_summary=df_gxe_summary[,c('K','Ne','Nu','Be','Bu','method','power','fdr')]
write.table(df_gxe_summary,paste0(odir, "SharePro_gxe_sim_gxe_summary.csv"),sep=',',row.names=F,col.names=T,quote=F)
df_gxe_summary <- read.table(paste0(odir, "SharePro_gxe_sim_gxe_summary.csv"),sep=',',header = T)
...
for (k in 1:3) {
  ggplot(df_gxe_summary[df_gxe_summary$K==paste0('K:',k),],
    ...
    #labs(color = "") 
  ggsave(paste0(odir,"SharePro_gxe_sim_gxe_summary",k,".pdf"),plt,height = 15, width = 10)
  ...
  ggsave(paste0(odir,"SharePro_gxe_sim_gxe_summary_fdr",k,".pdf"),plt,height = 15, width = 10)
...
timedt$method = unlist(lapply(strsplit(timedt$V1,'/'), function(x){x[3]}))
...
#ggplot(dat=timedf,aes(x=method,y=se,color = as.factor(method))) + geom_boxplot() + # + geom_boxplot(position = position_dodge(width = 0.6), width = 0.3) + 
#  theme_bw() + xlab("") + ylab("Runtime(s)") +
...
#  scale_color_manual(values = c("gold","darkgoldenrod1","darkblue")) +
#  labs(color = "") -> plt
#ggsave("~/scratch/SharePro_gxe/doc/SharePro_gxe_time.pdf",plt,height = 3,width = 5)
