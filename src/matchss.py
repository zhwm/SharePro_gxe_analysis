...
def preprocess(ss, args):
    ...
    if args.A1 is not None and args.A2 is not None:
        ...
        sys.exit('Please provide A1/A2 information')
    if args.log:
        ss[args.BETA] = np.log(ss[args.BETA])
    if args.rsid is not None and args.split:
        ss[args.rsid] = [i.split(':')[0] for i in ss[args.rsid]]  # for rsID:ALT:REF in GIANT
...
def match(ss, args, ch):
    """Load LD reference ID and match with GWAS"""
    ...
    ol = len(sub)
    sub = sub.loc[((sub['A2'] == idx['A2']) & (sub['A1'] == idx['A1'])) |
                  ((sub['A2'] == idx['A1']) & (sub['A1'] == idx['A2']))]
...
def merge(args):
    """Iterate through chromosomes to match GWAS summary statistics and LD alleles"""
    SS = [pd.read_csv(i, sep=args.delim, compression=args.compression) for i in args.rss]
    SS = [preprocess(i, args) for i in SS]  # Preprocess
    for ch in range(1, 23):
        matched = [match(i, args, ch) for i in SS]  # Match
        mid = pd.concat([i['SNP'] for i in matched], axis=1, join='inner').index  # Get index
        if len(mid) == 0:
            continue
        [matched[i].loc[mid, args.col].to_csv(os.path.join(args.save, "{}_{}.z".format(args.prefix[i], ch)),
                                              sep='\t', header=False, index=False) for i in range(len(matched))]
...
parser.add_argument('--POS', type=str, default=None, help='name of the position column in hg19')
