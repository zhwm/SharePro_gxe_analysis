...
def get_ld_ukb(ldfile, z, start, end, args):
    ...
    idx = sorted(set.intersection(*[set(z.index.intersection(i.index)) for i in df_R]))
    if len(idx) < 1000:
        print("less than 1000 variants matched, skipping")
        return [], [], [], [], [], [], [], []
    ...
    hess, varb = zip(*[get_HESS_h2_z(df_R[i].loc[idx, idx].values, Z[:, i], args.N[i]) for i in range(len(args.zdir))])
    return idx, effidx, XX, ytX, XtX, hess, varb, pvalvec
...
def ukb(args):
    ...
    print("{} summary statistics with {} common variants loaded at {}".
          format(z.shape[1], z.shape[0], time.strftime("%Y-%m-%d %H:%M")))
    ldlists = pd.read_csv(args.ukb, sep='\s+', dtype={'ld': str, 'start': int, 'end': int})
    print("LD list with {} LD blocks loaded\n".format(len(ldlists)))
    allv, allgamma = [], []
    cs, cs_prob, cs_share, cs_specific, cs_c, cs_eff, cs_pdiff, cs_tau, cs_gammasum = [], [], [], [], [], [], [], [], []
    for ite in range(len(ldlists)):
        ...
        for e in range(len(eff)):
            if eff[e][0] in effidx:
                ...
                print('causal variants in the {}-th effect group: {}'.format(e, mcs_idx))
                print('variant representation in this effect group: {}'.format(eff_gamma[e]))
                #print('shared probability of this effect group: {}'.format(eff_share[e]))
                #print('specific probabilities of this effect group: {}'.format(eff_specific[e]))
                #print('probabilities of effect group being causal: {}'.format(eff_c[e]))
                print('causal effect sizes: {}'.format(eff_mu[e]))
                ...
                cs_gammasum.append(eff_gammasum[e])
    ...
    allcs = pd.DataFrame({"cs": ['/'.join(i) for i in cs],
                          "totalProb": cs_gammasum,
                          "p_diff": ['{:.2e}'.format(i) for i in cs_pdiff],
...
parser.add_argument('--zdir', type=str, default=None, nargs='+', help='path to z-scores files', required=True)
...
parser.add_argument('--gwp', type=float, default=1e-5, help='genome-wide p value cutoff')
